...
void initMySess(HP2PSESS hsess)
{
...
//-----------------------------------------
int commandHandler(HP2PSESS hsess, CMDPKTINFO *pPkt)
{
...
PA_THREAD_RETTYPE __STDCALL mediaRecvThread(void *p)
{
	HP2PSESS hP2pSess = (HP2PSESS)p;
	MYSESSDATA *pSessData;
	pSessData = P2pSessGetUserData(hP2pSess);
...
int stopLiveVideo(HP2PSESS hsess, const char *s);
int openLiveAudio(HP2PSESS hsess, const char *s);
int stopLiveAudio(HP2PSESS hsess, const char *s);
int getFile(HP2PSESS hsess, const char *s);
int putFile(HP2PSESS hsess, const char *s);
	...
	while(1)
	{
		...
		else
		{
			...
			switch(line[0])
			{
				...
				case 'V': err = stopLiveVideo(hsess, ptr); break;
				case 'a': err = openLiveAudio(hsess, ptr); break;
				case 'A': err = stopLiveAudio(hsess, ptr); break;
				case 'g': err = getFile(hsess, ptr); break;
				case 'p': err = putFile(hsess, ptr); break;
...
int openLiveVideo(HP2PSESS hsess, const char *s)
{
	...
	if(pSessData->dwLiveMediaMask == 0)
		pSessData->thdLive = PA_ThreadCreate(mediaRecvThread, (void*)hsess);
