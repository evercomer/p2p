...
#ifdef WIN32
#define SETEVENT(event) SetEvent(event)
#else
...
static INLINE void _sendPacket(struct rudp_socket *s, struct rudp_channel *pch, struct rudp_pkt *pkt);
...
#ifdef WIN32
	sock->event_r = CreateEvent(NULL, FALSE, FALSE, NULL);
	sock->event_w = CreateEvent(NULL, FALSE, FALSE, NULL);
#else
...
#ifdef WIN32
	CloseHandle(s->event_r);
	CloseHandle(s->event_w);
#else
...
static void signalOutput(struct rudp_socket *s, int chno)
{
...
static int _RudpOutput(struct rudp_socket *s, int chno, int opt)
{
	...
	if(opt == RO_REXMT)	//packets are queued before
	{
		if(!psbuf->first) return 0;
		if(prbuf->should_ack == ACKT_DELAYED)
		{
			prbuf->should_ack = 0;
			psbuf->first->hdr.flags.ack = 1;
			psbuf->first->hdr.ackno = ntohl(prbuf->expected_seqno);
			psbuf->first->hdr.flags.n_loss = prbuf->n_lost;
			//if(psbuf->first->trans)	psbuf->first->hdr.crc32 = calc_crc32(0, (char*)&psbuf->first->hdr, offsetof(struct rudp_hdr, crc32));
		}
		_sendPacket(s, pch, psbuf->first);
		psbuf->first->hdr.flags.ack = 0;
		return 1;
	}
...
void _sendPacket(struct rudp_socket *s, struct rudp_channel *pch, struct rudp_pkt *pkt)
{
	if(pch->timer[RCT_REXMT] == 0)
...
_fastretr:
				if(psbuf->dup_ack == 3)
				{
					...
					_printTime(); dbg_msg("duplicated ACKs(%d): cwnd=%d, ssthresh=%d\n", ackno, s->pcb->cwnd, s->pcb->ssthresh);
				...
				{
					struct rudp_pkt *tmp = psbuf->first;
					psbuf->first = psbuf->rexmt;
					_RudpOutput(s, pkt->hdr.flags.chno, RO_REXMT);
				...
				else if(psbuf->dup_ack > 3)
				{
					s->pcb->cwnd ++;
#if 0
					_RudpOutput(s, pkt->hdr.flags.chno, /*RO_FORCE*/RO_ONLYONE);
#else
		...
		else// if(SEQ_GE(ackno, psbuf->first->seqno))
		{
			struct rudp_pkt *p, *p2;
			int passby = 0;	//fast retransmission
			...
			if( p ) //psbuf->first != NULL
				pch->timer[RCT_REXMT] = s->pcb->rto * rudp_backoff[psbuf->first->trans];
			else
			{
			...
			if(psbuf->dup_ack < 3)
			{
				if(psbuf->first && pch->congested)// && pkt->hdr.flags.n_loss)
				{
					psbuf->dup_ack = 3;
					goto _fastretr;
				}
			...
			if(old_rawnd == 0 && psbuf->rawnd > 0)	//Recevier's window opened
				...
				_RudpOutput(s, pkt->hdr.flags.chno, pch->congested?RO_REXMT:0);

			if(pch->congested && psbuf->rlost == 0/* && psbuf->n_unacked < (psbuf->rwin_size>>1)*/)
				pch->congested = 0;
...
_checkdata:
	if(pkt->len)	//length of data > 0
	{
		...
		if(delta < prbuf->q_size-1) //in the buffer
		{
			...
			//re-calculate the size of next hole.
			{
...
#if 0			/* Not used again */
			if(FD_ISSET(sockw_r, &rfds))
			{
				struct sockaddr sa;
				int salen;
				struct output_notify notify;
				salen = sizeof(sa);
				PA_RecvFrom(sockw_r, &notify, sizeof(notify), 0, &sa, &salen);

				//If we are in fast retransmission, let the fast-retransmission control
				//the sending speed(send only when an ack arrives)
				if(notify.s->state == RS_ESTABLISHED && notify.s->pcb && notify.s->pcb->channel[notify.chno].sbuf.dup_ack < 3)
				{
					PA_MutexLock(notify.s->mutex_w);
					//_RudpOutput(notify.s, notify.chno, 0);
					while(_RudpOutput(notify.s, notify.chno, 0) > 0);
					PA_MutexUnlock(notify.s->mutex_w);
				}
			}
#endif
		}
...
wait:
	...
	{
		PA_MutexUnlock(s->mutex_r);
		WaitForSingleObject(s->event_r, INFINITE);
		PA_MutexLock(s->mutex_r);
	}
#else
...
#ifdef WIN32
	WaitForSingleObject(s->event_w, INFINITE);
#else
	PA_MutexLock(s->mutex_w);
...
#ifdef WIN32
				PA_MutexUnlock(s->mutex_r);
				no_data = WaitForSingleObject(s->event_r, 
						timeout?timeout->tv_sec*1000+timeout->tv_usec/1000:INFINITE) == WAIT_OBJECT_0 ? 0 : 1;
				PA_MutexLock(s->mutex_r);
#else
				if(timeout)
...
#ifdef WIN32
			PA_MutexUnlock(s->mutex_w);
			writable = WaitForSingleObject(s->event_w, timeout?timeout->tv_sec*1000+timeout->tv_usec/1000:INFINITE) 
				== WAIT_OBJECT_0 ? 1 : 0;
			PA_MutexLock(s->mutex_w);
#else
			if(timeout)
...
int RUDPSelect(RUDPSOCKCHNO *r_rscs, int *n_rrscs, RUDPSOCKCHNO *w_rscs, int *n_wrscs, RUDPSOCKCHNO *e_rscs, int *n_erscs, const struct timeval *timeout)
{
	int i, ii, n;
       	int nr, nw, ne;
	struct list_head *p;
