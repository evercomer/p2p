...
static void MD5Encode (unsigned char *, UINT4 *, unsigned int);
static void MD5Decode (UINT4 *, unsigned char *, unsigned int);
...
void MD5Final (unsigned char* digest, MD5_CTX *context)
	...
	MD5Encode (bits, context->count, 8);  /* Pad out to 56 mod 64.*/
	index = (unsigned int)((context->count[0] >> 3) & 0x3f);
...
/* Encodes input (UINT4) into output (unsigned char). Assumes len is
  a multiple of 4. */
	static void MD5Encode (unsigned char *output, UINT4 *input, unsigned int len)
	{  
		unsigned int i, j;
...
/* Decodes input (unsigned char) into output (UINT4). Assumes len is
  a multiple of 4. */
	static void MD5Decode (UINT4 *output, unsigned char *input, unsigned int len)
		{  
			unsigned int i, j;
			for (i = 0, j = 0; j < len; i++, j += 4)
